...
class SparseGPRegression_fast(gp.models.SparseGPRegression):
    ...
    def __init__(self, X, y, kernel, Xu, noise=None, mean_function=None, 
        ...
        if mean_function is None: #not None:
            super().__init__(X, y, kernel, Xu, mean_function=None, 
        ...
        if precompute_inv == None:
            ...
            self.Luu = torch.linalg.cholesky(Kuu)#Kuu.cholesky()

            Kuf = self.kernel(self.Xu, self.X)

            W =  torch.linalg.solve_triangular(self.Luu, Kuf, upper = False) # Kuf.triangular_solve(self.Luu, upper=False)[0]
            D = self.noise.expand(N).to(device)

            self.W_Dinv = W / D
            K = self.W_Dinv.matmul(W.t()).contiguous()
            K.view(-1)[::M + 1] += 1  # add identity matrix to K
            self.L = torch.linalg.cholesky(K) #K.cholesky()
        else:
    ...
    def forward(self, Xnew, full_cov=False, noiseless=True, reuse_kernel=None):
        ...
        if reuse_kernel is None:           
            ...
            Ws = torch.linalg.solve_triangular(self.Luu, Kus, upper = False) #Kus.triangular_solve(self.Luu, upper=False)[0]
            reuse_kernel = Ws
            
        pack = torch.cat((W_Dinv_y, reuse_kernel), dim=1)
        Linv_pack = torch.linalg.solve_triangular(self.L, pack, upper = False) #pack.triangular_solve(self.L, upper=False)[0]
